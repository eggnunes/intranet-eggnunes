...
export default function Equipe() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, () => fetchTeam())
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'profiles' }, () => fetchTeam())
      .subscribe();
  ...
  const fetchTeam = async () => {
    ...
    if (!error && data) {
      const grouped: GroupedTeam = { socio: [], advogado: [], estagiario: [], comercial: [], administrativo: [] };
  ...
  const getPositionInfo = (position: keyof GroupedTeam) => {
    const info = {
      socio: { title: 'Sócios', icon: Briefcase, color: 'from-purple-500 to-violet-600', bgColor: 'bg-purple-50 dark:bg-purple-950/20', borderColor: 'border-purple-200 dark:border-purple-800' },
      advogado: { title: 'Advogados', icon: User, color: 'from-blue-500 to-cyan-600', bgColor: 'bg-blue-50 dark:bg-blue-950/20', borderColor: 'border-blue-200 dark:border-blue-800' },
      estagiario: { title: 'Estagiários', icon: GraduationCap, color: 'from-emerald-500 to-teal-600', bgColor: 'bg-emerald-50 dark:bg-emerald-950/20', borderColor: 'border-emerald-200 dark:border-emerald-800' },
      comercial: { title: 'Comercial', icon: Building2, color: 'from-orange-500 to-red-600', bgColor: 'bg-orange-50 dark:bg-orange-950/20', borderColor: 'border-orange-200 dark:border-orange-800' },
      administrativo: { title: 'Administrativo', icon: UserCog, color: 'from-pink-500 to-rose-600', bgColor: 'bg-pink-50 dark:bg-pink-950/20', borderColor: 'border-pink-200 dark:border-pink-800' },
    };
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <CardTitle className="text-lg mb-2 truncate flex items-center gap-2">
                            {member.full_name}
                            {isSocio && <ExternalLink className="h-3 w-3 text-muted-foreground" />}
                          </CardTitle>
                          <div className="space-y-1">
                            ...
                            {member.birth_date && (
                              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                <span>{format(parse(member.birth_date, 'yyyy-MM-dd', new Date()), "dd 'de' MMMM", { locale: ptBR })}</span>
                              </div>
                            )}
                            {member.join_date && (
                              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            {(member.oab_number || member.oab_state) && (
                              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                <span>OAB {member.oab_state && `${member.oab_state}/`}{member.oab_number}</span>
                              </div>
                            )}
                          </div>
                          {user && user.id !== member.id && (
                            <Button
                              ...
                              onClick={(e) => { e.stopPropagation(); startConversation(member.id, member.full_name); }}
                            >
